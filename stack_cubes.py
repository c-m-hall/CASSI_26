...
Also writes:
    - NCUBES_MAP: how many input cubes contributed (finite DATA) at each voxel
    - EXPTIME: summed EXPTIME (from each input cube's header) at each
      voxel, counting only cubes that actually contributed data there --
      a voxel masked out in one cube doesn't get that cube's exptime
      counted toward it, so this reflects effective/usable exposure time,
      not raw telescope integration time collected there.
...
def load_var_arrays(paths, ext=1):
    ...
    for every path. Returns a list of numpy arrays, same shape as the
    corresponding cube's data.
    """
    var_arrays = []
    for p in paths:
...
def get_exptime(path):
    """EXPTIME [s] from a cube's primary header, warning (and treating as
    0) if it's missing."""
...
def stack_cubes(cubes, paths, method="median"):
    ...
    n_map : numpy.ndarray
        ...
        as the data).
    exptime_map : numpy.ndarray
        Per-voxel summed EXPTIME, counting only cubes that contributed
        finite data at that voxel (same shape as the data).
    """
    data_stack = np.stack(
    ...
    var_out[n_var == 0] = np.nan   # no cube had a finite var here -- unknown, not zero
    var_out[all_nan] = np.nan      # DATA fully masked here -- keep VAR consistent with it

    # exptime: sum of each cube's EXPTIME header value, counting only
    # cubes that actually contributed finite data at that voxel (so a
    # cube masked out at a given voxel doesn't get its exptime counted
    # there, even though it does elsewhere in the cube)
    exptimes = np.array([get_exptime(p) for p in paths])
    exptime_per_cube = exptimes[:, None, None, None] * finite_stack
    exptime_map = exptime_per_cube.sum(axis=0)
    ...
    return primary, mean_cube, n_map, exptime_map
...
def main():
    ...
    primary, mean_cube, n_map, exptime_map = stack_cubes(cubes, paths, method=args.method)
    ...
    with fits.open(args.output, mode='update') as hdul:
        ...
        exptime_hdu = fits.ImageHDU(data=exptime_map.astype(np.float32), name='EXPTIME')
        exptime_hdu.header['COMMENT'] = ('summed EXPTIME [s] from cubes contributing '
                                          'finite data at each voxel')
        exptime_hdu.header['BUNIT'] = 's'

...
def main(sample_path='muse_x_milliquas_sample.fits',
    ...
    for row in sample:
        ...
        for cubepath in cube_paths:
            ...
            # already fully processed on a prior run? skip entirely, don't
            # even bother re-opening the cube -- this is what makes a
            # re-run after a Slurm time-limit kill cheap: only unfinished
            # cubes actually do any work.
            stem = os.path.splitext(cube_base)[0]
            already_done = glob.glob(os.path.join(DIR_FINAL, f'*{stem}*'))
            if already_done and resume_from == 'download':
                print(f"Already processed: {name} / {cube_base} -> "
                      f"{already_done[0]}, skipping.")
                continue

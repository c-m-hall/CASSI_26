...
def main():
    """Query Milliquas + ESO MUSE archive, cross-match, and write the
    field catalogue (fields, pairs) and QSO science sample to disk."""
    ...
    # Individual-cube-only stats, from pairs_indiv (excludes combined products,
    # SKY-offset exposures, and NFM cubes). download_sample.py needs THESE --
    # not the n_cubes/total_exptime/dp_ids above -- since those are the only
    # cubes actually worth queuing for download and running through
    # mainscript.py. Not every object in `fields` has an individual-cube
    # match, so this dict is looked up with .get(...) below and defaults to
    # "nothing downloadable" for objects missing from it entirely.
    indiv_stats = {}
    for g in pairs_indiv.group_by('Name').groups:
        indiv_stats[g['Name'][0]] = (
            len(g), float(g['t_exptime'].sum()), join_unique(g['dp_id']),
        )

    n_cubes_indiv, exptime_indiv, dp_ids_indiv = [], [], []
    for name in fields['Name']:
        n, exp, ids = indiv_stats.get(name, (0, 0.0, ''))
        n_cubes_indiv.append(n)
        exptime_indiv.append(exp)
        dp_ids_indiv.append(ids)

    fields['n_cubes_indiv'] = n_cubes_indiv
    fields['exptime_indiv'] = exptime_indiv * u.s
    fields['dp_ids_indiv'] = dp_ids_indiv
